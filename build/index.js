...
server.registerTool("shipcheck.audit", {
    description: "Audit a local Node/TypeScript package for npm publish readiness (read-only).",
    inputSchema: {
...
server.registerTool("shipcheck.packPreview", {
    description: "Show which files would be included by `npm pack` and report size totals (read-only).",
    inputSchema: {
        path: z.string().describe("Absolute or relative path to the package folder"),
    },
    annotations: { readOnlyHint: true },
...
server.registerTool("shipcheck.explainFailure", {
