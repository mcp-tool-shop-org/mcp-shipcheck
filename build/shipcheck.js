...
function add(findings, level, code, message, hint) {
    findings.push({ level, code, message, hint });
}
function finalize(root, name, version, findings) {
...
export async function auditPackage(path) {
    const root = resolve(path);
    const findings = [];
    const pkgPath = join(root, "package.json");
    const pkg = existsSync(pkgPath) ? safeReadJson(pkgPath) : null;
    if (!pkg) {
        add(findings, "fail", "PKG.MISSING", "package.json not found or invalid JSON.", "Run `npm init -y` (or fix JSON) in the target folder.");
    ...
    // 1) Basics
    if (!name)
        add(findings, "fail", "PKG.NAME.MISSING", "package.json is missing `name`.");
    if (!version)
        add(findings, "warn", "PKG.VERSION.MISSING", "package.json is missing `version`.");
    // 2) README / LICENSE
    ...
    const typesCandidate = typeof pkg.types === "string" ? pkg.types : typeof pkg.typings === "string" ? pkg.typings : null;
    if (!hasTypesField) {
        add(findings, "warn", "TS.TYPES.MISSING", "No `types`/`typings` field in package.json.", "If you publish TypeScript types, set `types` to your .d.ts entry (e.g. build/index.d.ts).");
    }
    ...
    // 4) Exports sanity
    if (!pkg.exports) {
        add(findings, "info", "PKG.EXPORTS.MISSING", "No `exports` map found.", "Consider adding `exports` to avoid deep-import chaos and to define ESM/CJS entrypoints explicitly.");
    }
...
export async function packPreview(path) {
    const root = resolve(path);
